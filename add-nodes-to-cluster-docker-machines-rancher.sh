...
function usage(){
	echo "add-nodes-to-cluster-docker-machine-rancher.sh {number_of_nodes} {project name} {project description} {template name} {set default project} {nodes_prefix} [-f]"
	...
	echo "Use: add-nodes-to-cluster-docker-machine-rancher.sh [-h|--help] for this help"
	echo "Use: add-nodes-to-cluster-docker-machine-rancher.sh [-l|--list] {nodes_prefix} for list of available projects"
}
...
if [[ $2 =~ '-' ]]; then
	...
	echo "Project Name: $2 -> invalid name format"
	echo "Expected format: {name}-{template}-env"
	echo -e "$(usage)"
	exit 1
fi
...
if [ -e $FOLDER/.settings/.${PREFIX}projects ]; then
	...
	if [ "" = "$EXISTS" ]; then
		echo "Project ${PROJECT_NAME} is not present in projects file, please choose another name!!"
		exit 1
	fi
fi


echo "Adding nodes to infrastructure -> prefix: $PREFIX type: $TEMPLATE_NAME ..."
...
PROJECT_ID="$(getProjectIdByProjectName "$RANCHER_MASTER_NODE_IP" "$PROJECT_NAME")"
if [ "" = "$PROJECT_ID" ] || [ "false" = "$(checkProjectId "$PROJECT_ID")" ]; then
	echo "No project on the rancher server with name: $PROJECT_NAME ..."
	exit 1
fi
TEMPLATE_NAME="$(getOrchestratorByProjectName "$RANCHER_MASTER_NODE_IP" "$PROJECT_NAME")"
if [ "" = "$TEMPLATE_NAME" ]; then
	echo "No template associated on Rancher Server with name ${TEMPLATE_NAME} for project ${PROJECT_NAME} (Id: ${PROJECT_ID} ..."
	exit 1
fi
